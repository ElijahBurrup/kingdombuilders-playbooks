"""Remove old v1/v2 auth-aware <script> blocks from every nav-bearing page so
the freshly-injected v3 is the only one running.
"""
import os
import re
...
for path in PAGES:
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_html)
    count += 1
    print(f"  stripped old auth-aware from {path}")
