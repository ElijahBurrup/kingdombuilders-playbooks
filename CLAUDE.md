...
6. **NO 3-column text-card grids**: Never lay out three text-heavy boxes side-by-side (`.char-grid`, `.protocol-grid`, `.pillar-grid`, `.darkroom-steps`, `.rules-trio`, `.spectrum-items`, `.taste-signal`, or any similar pattern with paragraph content). At desktop widths each column becomes a narrow strip with awkwardly wrapped text. Stack these vertically (single column, full width) so each card has the full page width to read in. 3-column layouts are fine ONLY for short stat strips (`.data-heroes`, `.stat-row` — one stat number + ~5-word label per cell), small button/state pickers, and tabular data rows. Anything with a `<p>` of meaningful prose goes single-column.
